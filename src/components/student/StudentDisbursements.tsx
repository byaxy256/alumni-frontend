...
interface StudentDisbursementsProps {
  user: User;
  onBack: () => void;
}
...
export function StudentDisbursements({ user, onBack }: StudentDisbursementsProps) {
  ...
  const handleDownload = async (tx: Transaction | null) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `disbursement-${tx.id}.pdf`;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          const errData = await res.json().catch(() => ({}));
          throw new Error(errData.error || 'Failed to load disbursements.');
        }
        ...
        toast.error(err?.message || 'Failed to load disbursements');
        setTransactions([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-6">
      <div className="bg-white border-b border-gray-200 p-4 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <h1 className="text-primary">Disbursements</h1>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="text-sm text-gray-500">No disbursements found.</div>
            )}
          </div>
        </div>
      </div>
