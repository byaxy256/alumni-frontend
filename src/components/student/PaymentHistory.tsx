...
export function PaymentHistory({ user, onBack }: { user: User; onBack: () => void; }) {
  ...
  const [phoneNumber, setPhoneNumber] = useState(user?.meta?.accessNumber || '');
  ...
  const handleMakePayment = async () => {
    ...
    if (paymentMethod === 'bank') {
        toast.success("Bank transfer details noted.", { description: "Please complete the transfer and use your Access Number as the reference." });
    ...
    <div className="min-h-screen pb-20 md:pb-6" style={{ background: 'var(--background)' }}>
      <div className="p-4 sticky top-0 z-10" style={{ background: 'linear-gradient(135deg, #0b2a4a 0%, #1a4d7a 100%)', borderBottom: '1px solid rgba(255,255,255,0.12)' }}>
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-white/15 rounded-lg text-white" title="Go back"><ArrowLeft className="w-5 h-5" /></button>
          <h1 className="text-white">Payment History</h1>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {paymentHistory.length > 0 ? (
              paymentHistory.map((payment) => {
                ...
                const studentAccessNumber = user?.meta?.accessNumber || 'N/A';
                return (
                    ...
                    <div className="flex gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-600">
                          ...
                          <span>•</span>
                          <span><strong>Access #:</strong> {studentAccessNumber}</span>
                        </div>
          ...
          <div className="space-y-4">
            ...
            {paymentMethod === 'bank' && (
              <div className="p-4 rounded-lg text-sm border" style={{ background: '#eef3ff', borderColor: '#c7d7f4' }}>
                ...
                <p className="mt-2 text-xs text-gray-600">Important: Use your Access Number as the payment reference.</p>
              </div>
            )}
            {(paymentMethod === 'mtn' || paymentMethod === 'airtel') && (
              <div>
                ...
                <Input
                  ...
                  maxLength={6}
                  disabled={phoneNumber === (user?.meta?.accessNumber || '')}
                />
                {phoneNumber === (user?.meta?.accessNumber || '') && (
                  <p className="text-xs text-gray-500 mt-1">Using your registered Access Number</p>
                )}
              </div>
            )}
