...
router.post("/", authenticate, authorize(['student']), upload.array("attachments"), async (req, res) => {
  try {
    const studentUid = (req as any).user.uid; // Get UID securely from token
    ...
    const [result] = await db.execute<ResultSetHeader>(
      "INSERT INTO support_requests (student_uid, amount_requested, reason, attachments, status) VALUES (?, ?, ?, ?, ?)",
      [ studentUid, Number(amountRequested), reason, JSON.stringify([]), 'pending' ] // Assuming attachments are handled separately
    );
...
router.get("/mine", authenticate, async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      "SELECT * FROM support_requests WHERE student_uid = ? ORDER BY created_at DESC",
      [studentUid]
    );
    res.json(rows || []);
  } catch (err) {
...
router.patch('/:id/status', authenticate, authorize(['alumni_office', 'admin']), async (req, res) => {
  ...
  try {
    ...
    await db.execute(
      'UPDATE support_requests SET status = ?, rejection_reason = ? WHERE id = ?',
