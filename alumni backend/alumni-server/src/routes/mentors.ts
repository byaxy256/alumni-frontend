...
router.get('/', async (req, res) => {
  try {
    ...
    for (const row of rows as RowDataPacket[]) {
      ...
      const pendingRequests: string[] = Array.isArray(meta.pending_requests) ? meta.pending_requests : [];
      const approvedMentees: string[] = Array.isArray(meta.approved_mentees) ? meta.approved_mentees : [];
...
router.get('/my-mentors', async (req, res) => {
  try {
    ...
    if (!payload?.uid) return res.status(401).json({ error: 'Invalid token payload' });

    // For students: get their approved mentors
    if (payload.role === 'student') {
      const [userRows] = await db.execute<RowDataPacket[]>(
        'SELECT meta FROM users WHERE uid = ? LIMIT 1',
        [payload.uid]
      );
      ...
      const approvedMentors: string[] = Array.isArray(userMeta.approved_mentors) ? userMeta.approved_mentors : [];
      ...
      const [mentorRows] = await db.execute<RowDataPacket[]>(
        `SELECT uid, full_name, meta FROM users WHERE uid IN (${placeholders})`,
        approvedMentors
      );
    ...
    const pendingRequests: string[] = Array.isArray(mentorMeta.pending_requests) ? mentorMeta.pending_requests : [];
...
router.post('/request', async (req, res) => {
  try {
    ...
    if (!studentUid) return res.status(401).json({ error: 'Invalid token payload' });
    if (role !== 'student') return res.status(403).json({ error: 'Only students can request mentors' });
    ...
    await db.execute(
      'UPDATE users SET meta = ? WHERE uid = ?',
      [JSON.stringify(updatedMentorMeta), mentorId]
    );
    ...
    if (Array.isArray(studentRows) && studentRows.length > 0) {
      ...
      const pendingMentors: string[] = Array.isArray(studentMeta.pending_mentors) ? studentMeta.pending_mentors : [];
      ...
      await db.execute(
        'UPDATE users SET meta = ? WHERE uid = ?',
        [JSON.stringify(updatedStudentMeta), studentUid]
      );
    }
...
router.post('/approve', async (req, res) => {
  try {
    ...
    if (!mentorUid) return res.status(401).json({ error: 'Invalid token payload' });
    if (role !== 'alumni') return res.status(403).json({ error: 'Only alumni can approve requests' });

    const [rows] = await db.execute<RowDataPacket[]>(
      'SELECT full_name, meta FROM users WHERE uid = ? LIMIT 1',
      [mentorUid]
    );
    ...
    await db.execute(
      'UPDATE users SET meta = ? WHERE uid = ?',
      [JSON.stringify(updatedMeta), mentorUid]
    );
    ...
    if (Array.isArray(studentRows) && studentRows.length > 0) {
      const studentMeta = parseMeta(studentRows[0].meta);
      const pendingMentors: string[] = Array.isArray(studentMeta.pending_mentors) ? studentMeta.pending_mentors : [];
      const approvedMentors: string[] = Array.isArray(studentMeta.approved_mentors) ? studentMeta.approved_mentors : [];
      ...
      await db.execute(
        'UPDATE users SET meta = ? WHERE uid = ?',
        [JSON.stringify(updatedStudentMeta), studentId]
      );
    }
    ...
    await db.execute(
      ...
      [studentId, 'Mentor request approved', `${mentorName} approved your mentorship request.`]
    );
...
router.post('/reject', async (req, res) => {
  try {
    ...
    if (!mentorUid) return res.status(401).json({ error: 'Invalid token payload' });
    if (role !== 'alumni') return res.status(403).json({ error: 'Only alumni can reject requests' });
    ...
    await db.execute(
      'INSERT INTO notifications (target_uid, title, message) VALUES (?, ?, ?)',
      [studentId, 'Mentor request declined', `${mentorName} declined your mentorship request.`]
    );
...
router.post('/remove-approved', async (req, res) => {
  try {
    ...
    if (!mentorUid) return res.status(401).json({ error: 'Invalid token payload' });
    if (role !== 'alumni') return res.status(403).json({ error: 'Only alumni can remove mentees' });
...
router.get('/my-approved-mentees', async (req, res) => {
  try {
    ...
    const mentorMeta = parseMeta(mentorRows[0].meta);
    const approvedMentees: string[] = Array.isArray(mentorMeta.approved_mentees) ? mentorMeta.approved_mentees : [];
