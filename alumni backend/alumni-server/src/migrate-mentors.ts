...
async function migrateMentorData() {
  ...
  try {
    ...
    const [alumniRows] = await db.execute<RowDataPacket[]>(
      'SELECT uid, full_name, meta FROM users WHERE role = ?',
      ['alumni']
    );
    ...
    for (const alumni of alumniRows) {
      ...
      const approvedMentees: string[] = Array.isArray(alumniMeta.approved_mentees) 
        ? alumniMeta.approved_mentees 
        : [];

      if (approvedMentees.length === 0) continue;
      ...
      for (const studentId of approvedMentees) {
        const [studentRows] = await db.execute<RowDataPacket[]>(
          'SELECT uid, full_name, meta FROM users WHERE uid = ?',
          [studentId]
        );
        ...
        const approvedMentors: string[] = Array.isArray(studentMeta.approved_mentors) 
