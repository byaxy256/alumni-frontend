...
const mapEventRow = (row) => ({
    ...
    description: row.description ?? row.content ?? '',
    content: row.content ?? '',
    hasImage: !!(row.image_data || row.image_url || row.hasImage),
...
router.get('/news', async (req, res) => {
    try {
        ...
        if (audience) {
            try {
                const [rows] = await db.execute("SELECT * FROM news WHERE status = 'published' AND (audience = ? OR audience = 'both' OR audience IS NULL) ORDER BY created_at DESC", [audience]);
        ...
        try {
            const [rows2] = await db.execute('SELECT * FROM news ORDER BY id DESC LIMIT 100');
...
router.get('/events', async (req, res) => {
    try {
        ...
        if (audience) {
            try {
                const [rows] = await db.execute("SELECT e.*, (SELECT COUNT(*) FROM event_registrations er WHERE er.event_id = e.id) AS attendees FROM events e WHERE (e.audience = ? OR e.audience = 'both' OR e.audience IS NULL) ORDER BY (e.event_date IS NULL), e.event_date ASC, e.id DESC LIMIT 100", [audience]);
                return res.json({ content: rows.map(r => ({ ...mapEventRow(r), attendees: r.attendees || 0 })) });
            }
            catch (errFilter) {
                console.error('GET /content/events audience filter error', errFilter);
            }
        }
        // Avoid referencing non-existent columns; order by date if available
        const [rows] = await db.execute('SELECT e.*, (SELECT COUNT(*) FROM event_registrations er WHERE er.event_id = e.id) AS attendees FROM events e ORDER BY (e.event_date IS NULL), e.event_date ASC, e.id DESC LIMIT 100');
        res.json({ content: rows.map(r => ({ ...mapEventRow(r), attendees: r.attendees || 0 })) });
    }
    catch (err) {
...
// GET image bytes for news
router.get('/news/:id/image', async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await db.execute('SELECT image_data, image_mime FROM news WHERE id = ? LIMIT 1', [id]);
        const row = rows[0];
...
router.get('/events/:id/image', async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await db.execute('SELECT image_data, image_mime, image_url FROM events WHERE id = ? LIMIT 1', [id]);
        const row = rows[0];
        if (!row)
            return res.status(404).json({ message: 'Image not found' });
...
router.post('/news', authenticate, authorize(['admin', 'alumni_office']), upload.single('image'), async (req, res) => {
    try {
        ...
        const author_id = req.user?.uid ?? null;
        ...
        try {
            const [result] = await db.execute('INSERT INTO news (title, content, image_data, image_mime, status, audience) VALUES (?, ?, ?, ?, ?, ?)', [title, contentVal, file?.buffer ?? null, file?.mimetype ?? null, published ? 'published' : 'draft', audience]);
            return res.status(201).json({ id: result.insertId, message: 'News article created successfully.' });
        }
        catch (e1) {
            // Fallback without image columns
            const [result] = await db.execute('INSERT INTO news (title, content, status) VALUES (?, ?, ?)', [title, contentVal, published ? 'published' : 'draft']);
            return res.status(201).json({ id: result.insertId, message: 'News article created successfully.' });
        }
    }
    catch (err) {
...
router.put('/news/:id', authenticate, authorize(['admin', 'alumni_office']), upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        if (file) {
            try {
                await db.execute('UPDATE news SET title = ?, content = ?, image_data = ?, image_mime = ?, status = ?, audience = ? WHERE id = ?', [title, contentVal, file.buffer, file.mimetype, published ? 'published' : 'draft', audience, id]);
            }
            catch (e1) {
                // Fallback without image columns
                await db.execute('UPDATE news SET title = ?, content = ?, status = ? WHERE id = ?', [title, contentVal, published ? 'published' : 'draft', id]);
            }
        }
        else {
            await db.execute('UPDATE news SET title = ?, content = ?, status = ? WHERE id = ?', [title, contentVal, published ? 'published' : 'draft', id]);
...
router.post('/events', authenticate, authorize(['admin', 'alumni_office']), upload.single('image'), async (req, res) => {
    try {
        ...
        try {
            const [result] = await db.execute('INSERT INTO events (title, description, event_date, event_time, location, registration_fee, image_url, status, audience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [title, description, date, time || null, location || '', fee, imageUrl, published ? 'published' : 'draft', audience]);
            return res.status(201).json({ id: result.insertId, message: 'Event created successfully.' });
        }
        catch (e1) {
            const [result] = await db.execute('INSERT INTO events (title, description, event_date, event_time, location, registration_fee, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [title, description, date, time || null, location || '', fee, published ? 'published' : 'draft']);
            return res.status(201).json({ id: result.insertId, message: 'Event created successfully.' });
        }
    }
    catch (err) {
...
router.put('/events/:id', authenticate, authorize(['admin', 'alumni_office']), upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        const imageUrl = await saveUploadedFile(file);
        if (imageUrl) {
            await db.execute('UPDATE events SET title = ?, description = ?, event_date = ?, event_time = ?, location = ?, registration_fee = ?, image_url = ?, status = ? WHERE id = ?', [title, description, date, time || null, location || null, fee, imageUrl, published ? 'published' : 'draft', id]);
        }
        else {
            await db.execute('UPDATE events SET title = ?, description = ?, event_date = ?, event_time = ?, location = ?, registration_fee = ?, status = ? WHERE id = ?', [title, description, date, time || null, location || null, fee, published ? 'published' : 'draft', id]);
        }
...
router.post('/events/:id/register', authenticate, authorize(['student']), async (req, res) => {
    ...
    try {
        ...
        const [eventRows] = await db.execute('SELECT registration_fee FROM events WHERE id = ? LIMIT 1', [id]);
        ...
        if (fee > 0) {
          ...
          created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            ...
            const [payRows] = await db.execute('SELECT id FROM event_payments WHERE event_id = ? AND user_uid = ? AND status = ? LIMIT 1', [id, studentUid, 'SUCCESS']);
            if (!payRows || payRows.length === 0) {
                return res.status(400).json({ message: 'Payment required before registration.' });
            }
        }
        // Check if already registered
        const [existing] = await db.execute('SELECT id FROM event_registrations WHERE event_id = ? AND student_uid = ?', [id, studentUid]);
        if (existing && existing.length > 0) {
            return res.status(400).json({ message: 'Already registered for this event' });
        }
        // Insert registration
        await db.execute('INSERT INTO event_registrations (event_id, student_uid, registered_at) VALUES (?, ?, NOW())', [id, studentUid]);
...
router.get('/events/:id/registrations', authenticate, authorize(['admin', 'alumni_office']), async (req, res) => {
    const { id } = req.params;
    try {
        const [registrations] = await db.execute(`SELECT er.id, er.student_uid, er.registered_at, u.email, u.full_name
...
router.post('/', authenticate, authorize(['admin', 'alumni_office']), async (req, res) => {
    ...
    if (type === 'news') {
        ...
        try {
            const author_id = req.user?.uid ?? null;
            const [result] = await db.execute('INSERT INTO news (title, content, author_id, target_audience, status) VALUES (?, ?, ?, ?, ?)', [title, description || content || '', author_id, target_audience || 'all', published ? 'published' : 'draft']);
            return res.status(201).json({ id: result.insertId, message: 'News article created successfully.' });
        }
        catch (err) {
    ...
    if (type === 'event' || type === 'events') {
        ...
        try {
            const [result] = await db.execute('INSERT INTO events (title, description, event_date, status) VALUES (?, ?, ?, ?)', [title, description, date, published ? 'published' : 'draft']);
            return res.status(201).json({ id: result.insertId, message: 'Event created successfully.' });
        }
        catch (err) {
